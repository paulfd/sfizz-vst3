...
This is a tentative implementation of the sfizz sfz sampler (https://github.com/sfztools/sfizz) as a VST3 plugin.
I am still having a hard time handling the API properly, so consider it a large work in progress.
For now the sfz file loaded is hardcoded in the source for example.

Working:
- Basic integration
- Note on/off dispatching
- Midi CC dispatching (is there a better way to do this other than creating 2000 fake parameters?)

Missing:
- A basic GUI to load arbitrary SFZ files
- Saving and restoring the plugin state
